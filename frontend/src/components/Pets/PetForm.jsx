    ...
    <form onSubmit={handleSubmit} className="space-y-7">
      ...
      <div>
        <div className="flex items-center gap-2 mb-4">
          <div className="w-6 h-6 rounded-full bg-brand-500 flex items-center justify-center text-xs font-bold text-white shrink-0">1</div>
          <h3 className="font-semibold text-white">¿De quién es esta mascota?</h3>
        </div>
          ...
          <button
            ...
            className={`flex-1 py-2.5 rounded-xl border text-sm font-medium transition-all ${
              ownerMode === 'select'
                ...
                : 'bg-surface-700/50 border-white/5 text-white/40 hover:text-white'
            }`}
          >
        ...
        {ownerMode === 'select' ? (
          <div className="space-y-2">
            ...
            {(ownerSearch || !ownerId) && filteredOwners.length > 0 && !selectedOwner && (
              <div className="bg-surface-700 border border-white/10 rounded-xl overflow-hidden max-h-48 overflow-y-auto">
                {filteredOwners.map(o => (
                  <button
                    key={o.id}
                    type="button"
                    onClick={() => { setOwnerId(String(o.id)); setOwnerSearch(''); setErrors(er => ({ ...er, owner: '' })); }}
                    className="w-full flex items-center gap-3 px-4 py-3 hover:bg-surface-600 transition-colors text-left"
                  >
                    <div className="w-8 h-8 rounded-lg bg-violet-500/20 flex items-center justify-center text-violet-300 font-bold text-sm shrink-0">
                      {o.name.charAt(0)}
                    </div>
                    <div>
                    ...
                    <span className="ml-auto text-white/30 text-xs">{o.pets?.length || 0} mascota(s)</span>
                  </button>
                ))}
              </div>
            )}
            {selectedOwner && (
              <div className="flex items-center gap-3 bg-brand-500/10 border border-brand-500/30 rounded-xl px-4 py-3">
                ...
                <div className="flex-1">
                  <p className="text-brand-300 font-medium text-sm">{selectedOwner.name}</p>
                  {selectedOwner.phone && <p className="text-white/40 text-xs">{selectedOwner.phone}</p>}
                </div>
                <button type="button" onClick={() => setOwnerId('')} className="text-white/30 hover:text-white">
                  <X size={14} />
                </button>
              </div>
            )}
            {ownerSearch && filteredOwners.length === 0 && (
              <p className="text-white/30 text-sm text-center py-3">
                ...
                <button type="button" onClick={() => setOwnerMode('new')} className="text-brand-400 underline">
                  Crear nuevo
                </button>
              </p>
            )}
          ...
          <div className="bg-surface-700/50 border border-white/5 rounded-xl p-4 space-y-3">
            <p className="text-white/50 text-xs mb-2 flex items-center gap-1.5"><User size={12} /> Datos del nuevo propietario</p>
        ...
        {errors.owner && <p className="text-red-400 text-xs mt-1.5">{errors.owner}</p>}
      </div>

      {/* ───── PASO 2: Datos básicos de la mascota ───── */}
      <div>
        <div className="flex items-center gap-2 mb-4">
          <div className="w-6 h-6 rounded-full bg-brand-500 flex items-center justify-center text-xs font-bold text-white shrink-0">2</div>
          <h3 className="font-semibold text-white">Información de la mascota</h3>
        </div>
              ...
              <button
                ...
                className={`flex-1 py-3 rounded-xl border text-sm font-medium transition-all flex flex-col items-center gap-1 ${
                  form.species === s.value
                    ? 'bg-brand-500/15 border-brand-500/40 text-brand-300'
                    : 'bg-surface-700/50 border-white/5 text-white/40 hover:text-white hover:border-white/20'
                }`}
              >
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2 sm:col-span-1">
            ...
            <input className={`input-field ${errors.name ? 'border-red-500' : ''}`} value={form.name} onChange={e => set('name', e.target.value)} placeholder="Ej: Rocky" />
          ...
          <div className="flex flex-wrap gap-2">
            {BEHAVIORS.map(b => (
              <button key={b.value} type="button" onClick={() => set('behavior', form.behavior === b.value ? '' : b.value)}
                ...
                {b.emoji} <span className="capitalize text-xs">{b.value}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-semibold text-white">Alertas y cuidados especiales</h3>
        </div>
        <div className="bg-red-500/5 border border-red-500/30 rounded-xl p-4">
          ...
          <textarea
            className="w-full px-4 py-3 bg-red-500/5 border border-red-500/20 rounded-xl text-white text-sm 
                       placeholder-red-300/30 outline-none focus:border-red-400 focus:ring-2 focus:ring-red-400/20 resize-none"
            rows={3}
            value={form.alerts}
          ...
          />
          <p className="text-red-300/40 text-xs mt-1.5">Esta información es visible en la ficha del servicio y el dashboard.</p>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          <div className="w-6 h-6 rounded-full bg-surface-500 flex items-center justify-center text-xs font-bold text-white/60 shrink-0">4</div>
          <h3 className="font-semibold text-white/60">Foto de perfil <span className="text-white/30 text-xs font-normal">(opcional)</span></h3>
        </div>
        <div className="flex items-center gap-4">
          {form.photo ? (
            <div className="relative">
              <img src={form.photo} className="w-24 h-24 rounded-xl object-cover border border-white/10" alt="preview" />
            ...
            <div className="w-24 h-24 rounded-xl bg-surface-700 border-2 border-dashed border-white/10 flex items-center justify-center">
